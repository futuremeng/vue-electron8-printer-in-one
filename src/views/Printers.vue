...
LastEditTime: 2020-09-08 16:47:27
-->
<template>
  <div class="printers">
      ...
      >
        <el-card :body-style="{ padding: '0px' }">
          <div style="padding: 14px;">
            ...
            <el-tag
              >{{ printer.status
              }}{{ printer.status === 0 ? "闲置" : "脱机" }}</el-tag
            >
            <div class="bottom clearfix">
              <el-button class="button" @click="handleGetPrinter"
                >详情</el-button
              >
              <el-button class="button" @click="handleTestPrinter"
                >测试</el-button
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleTestPrinter() {
      console.log("clicked testPrinter");
      ipcRenderer.send("testPrinter");
      ipcRenderer.once("testPrinter", (event, data) => {
        // 过滤可用打印机
        // this.printers = data.filter((element) => element.status === 0);
        // this.printers = data;
        // console.log(this.printers);
